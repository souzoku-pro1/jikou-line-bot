...
class Base(unittest.IsolatedAsyncioTestCase):
    ...
    def patches(self, parse_results, hits=None, existing=None, create_ids=("101",)):
        seq = parse_results if isinstance(parse_results, list) else [parse_results]
        self.create_mock = AsyncMock(return_value=list(create_ids))
        ...
        return [
            ...
            patch("hub.kintone.create_records", new=self.create_mock),
        ]
...
class TestFullCycle(Base):
    async def test_instruction_to_filing(self):
        ...
        fields = self.create_mock.await_args.args[1][0]
