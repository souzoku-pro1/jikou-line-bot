...
| ② | 無効化行の下流除外の実装単位 | (A) 各 consumer へ横展開 (B) 共通ヘルパ一点除外 | **RESOLVED＝(B)**（fix5 裁定——単一の正の確立パターン。**fix6・RV08-05 明記: (B)→(A) への変更は本 DRAFT の設計改定＋司令塔の再裁定を要する**——実装票の単独判断で切り替えない） |
...
| ⑥ | **App34 ID 保持箇所の全数と保存先の意味論**（fix1・RV08-01） | rg 全数調査＋履歴/current/種類別の分類・**調査完了が凍結条件** | **RESOLVED＝§10 に全数調査を収載（fix5・本票内で実施）**。App36 `導出元人物ID` は**「履歴として敗者 ID を残す＝付け替えない」を既定方針**。個別論点は **§10.2 の確定事項**（~~§10 の選択肢欄~~・fix7/RV08-06 文言追随——fix6 で確定済みのため。凍結条件は**充足**） |
...
### 10.2 個別論点（fix6・RV08-05 で**確定事項**へ変更——実装票への先送りを解消）

- **(i) App36 導出元人物ID が敗者 ID のまま残る帰結＝確定**: shokumu_plan §2B-5
  （App36 の person 集合×run payload の一致検証）は敗者 ID 残置で**不一致＝条件
  未充足へ安全側に落ちる**（自然遮断・実測済みの 6 条件設計）。
  **自然遮断のみで足りるとする。追加の可視化監視は別票**（本票スコープ外）。
- **(ii) 未クローズ封筒の保留人物ID＝確定**: 再確定（resumed 経路）の phase 1
  再検証で当該 person が無効化行なら**要確認へ倒す**（RV08-03 の「直接 get の
  状態確認」規約に含める）。**封筒 detail の書き換えは禁止**（履歴改変になる）。
- **(iii) koseki_person_sync の冪等キー＝確定**: 無効化行が冪等ヒットした場合は
  **再生成を抑止（現行挙動の維持）**——重複人物の再出現を防ぐ。無効化後の
  再生成を許す変更は行わない。

（撤回注記: fix5 時点の「選択肢提示・実装票で確定」は D6・RV08-05 の確定要求を
受け上記へ確定。不採用の対案〔(i) 可視化検査の本票内追加・(ii) detail 書き換え・
(iii) 有効行のみ照合〕は理由付きで本注記に残置）
...
## 12. fix6 改定記録（R-DOCS-BATCH-1-D6・2026-08-11・RV08-05）

- **RV08-05**: §10.2 の個別論点3件を「選択肢提示・実装票で確定」から**確定事項**へ
  変更——(i) 自然遮断のみ・追加監視は別票 (ii) resumed phase 1 要確認・封筒 detail
  書き換え禁止 (iii) 無効化行冪等ヒット=再生成抑止。不採用対案は撤回注記へ残置。
- 裁定②へ「**(B)→(A) 変更は設計改定＋司令塔再裁定を要する**」を明記（実装票の
  単独判断による切替経路を閉鎖）。

## 13. fix7 改定記録（R-DOCS-BATCH-1-D7・2026-08-11・RV08-06=文言追随のみ）

- **RV08-06（LOW）**: §6 裁定⑥の「個別論点は §10 の選択肢欄」を「**§10.2 の
  確定事項**」へ文言追随（fix6・RV08-05 で確定済みの状態と表の記述を同期。
  本文設計は不変・撤回は取り消し線残置）。

## 14. 凍結記録（2026-08-11・R-DOCS-BATCH-1-D8 PASS）

- **巡歴**: 起草（DOCS-BATCH-1 A）→ D1（RV08-01/02/03・fix1）→ D2（RV08-04・
  fix2）→ D3〜D5（DESIGN_OK・fix5 で裁定②〜⑦ RESOLVED＋§10 全数調査収載）→
  D6（RV08-05・fix6）→ D7（RV08-06・fix7 文言追随）→ **D8 PASS＝本凍結**。
- **凍結条件充足の要旨**: 裁定①〜⑦全件 RESOLVED・裁定⑥の凍結条件（App34 ID
  保持箇所の rg 全数調査の収載）は §10 で充足・§10.2 個別論点は確定事項化済み。
- **以後の変更は設計改定＋司令塔再裁定を要する**（裁定②の変更経路閉鎖と同じ
  規律を本 DRAFT 全体に適用）。実装着手ゲート（[人]: App34 フィールド3件 CU 追加・
  TOKEN_KOSEKI_PERSON 削除権限の実権限確認/除去）は凍結条件ではなく実装前提。
