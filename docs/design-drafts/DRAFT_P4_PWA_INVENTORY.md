...
| 画面候補 | データ源 | kintone/DB アクセス形 | 直近スキャン一覧（※） |
|---|---|---|---|
| 案件一覧 | App21（案件） | **複数レコード検索**（status 別・更新順） | 不要 |
| 案件詳細 | App21 単票＋App30（発送管理・案件絞込）＋App28（チャットログ・件数程度） | 単票＋絞込検索 | 不要 |
| 相続人関係図 | App34（人物）＋App33（戸籍）＋（P3 merge 後）derivation_run／heir_confirmation_decision の projection | 検索＋DB 読取。描画は kinship_graph→kinship_renderer 流用（サーバ側で画像生成→PWA は img 表示が最小構成） | 不要 |
| 書類到着状況 | **ingestion_receipt**（file ingest の可視化台帳・last_outcome が state 正本）＋inbound_event（webhook 系） | **DB 読取のみ**（kintone 不要） | **要**（※） |
| 承認キュー参照 | App29（承認キュー） | 複数レコード検索（未送信/承認待ちの絞込） | 不要 |

- （※）**「scan 20件」の解釈**: 票の「scan 20件が要る画面」を**「直近のスキャン受領
  20 件の一覧参照（ingestion_receipt/inbound_event 由来）が要る画面」**と解釈した。
  該当は「書類到着状況」のみで、他 4 画面は不要（§5 の票分割に反映）。
  **この解釈が v2.4 の意図と一致するかは[人]確認事項**。
...
| P4-003: 書類到着状況 | ingestion_receipt/inbound_event の直近受領一覧 API（**直近 20 件・※解釈は §2**）＋画面。DB 読取のみ | **要** | P4-001（kintone 非依存） |
...
- 依存整理: **P3 merge を待つのは P4-005 の導出結果重畳のみ**。P4-001〜004 は現行
  main の資産（kintone proxy＋ingestion_receipt）だけで完結する。
- 順序案: P4-001 → 002/003 並行 → 004 → 005。
