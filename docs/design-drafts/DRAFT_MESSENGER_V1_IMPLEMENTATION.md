...
- **依頼/所見の完全性保証（fix1・P2MSG-H02）**:
  ...
  - **【fix2・P2MSG2-H01】所見の完全性保証の再設計**:
    - **self-hash は「破損検出用途に限定」へ格下げ**（搬送中の欠落・文字化けの検出のみ）。
      **改変検出には使えない**: 所見を編集できる主体（PC-A）は**ハッシュも再計算して
      付け直せる**ため、self-hash は編集に対する保証を一切与えない。
    - **v1 必須条件＝「PC-A 書込不可の outbox」**: 具体案は **Codex 実行ユーザーのみ
      書込可能な OS 権限ディレクトリ**（PC-A プロセスは読取のみ・ACL で強制）。
      司令塔は「PC-A が書けない場所から読む」ことで無編集を構造的に担保する。
    - **必須 fallback（明文化）**: 上記の書込不可 outbox が**実現不能な環境では
      codex_relay を実施しない**（[人] 貼付方式を維持）。「保証なしでの自動搬送」への
      縮退は行わない。
    - 代替案（**将来案として列挙のみ**・v1 対象外）: Codex 署名鍵（所見への署名）・
      司令塔直接配送（PC-A 非経由チャネル）。
  - **PC-A の編集検出/抑止の整理（fix2）**: (i) **抑止の本命＝書込不可 outbox**（上記）
    (ii) self-hash＝破損検出のみ (iii) 依頼と所見の TASK_ID／SHA 突合
    (iv) 司令塔の抜き打ち再実行依頼（同一 SHA・同一契約で再現照合）。
...
- **前提（[人]確認）**: GAS プロジェクトへの clasp 認証（Google アカウント・
  Apps Script API 有効化）は [人] 工程。
- **【fix2・P2MSG2-M01】第2段の必須 fallback（計画本文に固定）**: **Apps Script API で
  実効 read-only credential が成立しない場合、第2段（PC-A pull）は実施せず、
  [人] snapshot 方式（`tools/gas_drift_check.py` 運用）を維持する**。
  「read-only が担保できないまま PC-A に clasp credential を置く」構成へは進まない。
  read-only 成立可否の導入時確認は **OPEN-C** として起票（§6）。
...
## 6. OPEN 一覧（fix2 整理・いずれも司令塔/[人]裁定）

| ID | 内容 | 関連 |
|---|---|---|
| OPEN-A | Codex CLI の実行主体（PC-A 起動 or [人] 起動・FROZEN「搬送のみ」の解釈） | §1 |
| OPEN-B | Codex 実行環境の機械強制の実現手段選定（CLI sandbox／OS 権限・別ユーザー／コンテナ） | §1 |
| OPEN-C | Apps Script API の実効 read-only credential 成立可否（不成立なら第2段は実施せず [人] snapshot 方式維持） | §3 |
