...
- 追記: 2026-09-22 ── 裁定 B・G-2・G-3 の正文（§1-3・§1-5・§1-6。出典: 大野決定 2026-09-22（司令塔経由））、裁定 B-1 の要旨（§1-5）。
...
**裁定 B-1（大野決定・2026-09-22）｜正規化表の承認結果** ── 要旨（表の本体は `hub/intake_choice_synonyms.json`・`_meta.status`=「承認済み（裁定B-1・2026-09-22）」）

- 続柄: 意味が一意に決まる言い換えのみ承認（息子・娘・長男・長女等→子、妻・夫・奥様・ご主人等→配偶者、父・母・お父様・お母様等→親、兄・姉・弟・妹等→兄弟姉妹）。孫・甥・姪・叔父・叔母・いとこ・継子・配偶者の親・その他への寄せは不承認（表に入れない=停止）。kintone の選択肢は当面追加しない。
- 本人区分: 意味が一意な言い換えのみ（本人です→本人 等）。曖昧な表現は入れない。
- 相続順位: 表記ゆれのみ（第一順位・1・一→第1順位、第二順位・2→第2順位、第三順位・3→第3順位）。追加検査: 続柄と相続順位の両方が埋まっている場合、法定の対応（子→第1順位、親→第2順位、兄弟姉妹→第3順位、配偶者→順位なし）と食い違えばスキーマ逸脱と同じ扱いで応答全体を停止し、App 28 に食い違いの内容を記録する。どちらか一方のみの場合は検査しない。
- 同時申述希望: 意味が一意な言い換えのみ（希望します・お願いします→はい、不要です・希望しません→いいえ 等）。
- 財産処分有無: あり／なしの明示的な言い換え（有・無 等）のみ。事実からの推測（使った・解約した→あり 等）は不承認（法律判断のため停止して人が見る）。
- 訴訟督促有無: 本欄は裁判手続（訴訟・支払督促）の有無。催告書・請求書の受領は無関係。あり／なしの明示的な言い換えのみ。事実からの推測（催告書が来た・通知が来た→あり 等）は不承認。
- kintone の選択肢名との対応（実装時の読み替え・`_meta.kintone_value_map`）: 「親」=続柄「直系尊属（父母・祖父母）」、「第1順位／第2順位／第3順位」=相続順位「子／直系尊属／兄弟姉妹」、「はい／いいえ」=同時申述希望「あり／なし」。
- 実装: `hub/human_reply_intake.RANK_BY_RELATION`・`_rank_mismatch`、`test_hri04_choice_policy.py` `TestRulingB1`
