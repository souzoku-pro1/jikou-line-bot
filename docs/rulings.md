...
### 1-3. 「裁定 G-2」（大野決定・2026-09-20） ── 原文あり（指示票の逐語を PR #257 本文へ転記）
...
### 1-4. 「裁定 A」「裁定 B」「裁定 C」（HRI） ── 未発見

- リポジトリ・全 PR 本文・作業メモ・設計書のいずれにも、この名前の付いた文言は無い。
- 指示票の要約として伝わっている内容:
  - C: 「取込経路に適用する上限を「初回＋再試行1回、再取得1回」に設定する（裁定C）」（HRI-05 の票）
  - B: 「裁定Bの適用方針が未確定」（HRI-04 の票。選択肢 enum 外値の扱い）
  - A: 内容の記載なし
...
- HRI「裁定 A」: どこにも出現しない
- HRI「裁定 B」「裁定 C」: この名前の付いた文言は無い（§1-4 の名前なしの文言のみ）
- HRI「裁定 G」の原文
