...
**DEFER 記録（2026-08-17・司令塔裁定）**: R-Q 系レビューの LOW 2 件 **Q-03・Q-04** は DEFER（本実装への反映なし・所見の実体はレビュー票が正本）。再開はいずれかの Q 系後続票への相乗りで消化する。あわせて Q-BATCH-1 の運用前提: **migration `c4e8a2d6b9f1`（qa_record）の本番適用（[人]・確立手順）が merge 後に必要**——適用まで質問画面の台帳表示は接続エラーとなる（質問機能の他の regressions はなし）。
