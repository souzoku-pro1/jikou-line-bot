...
#### T3-3 M1 結線（発送済→返送待ち→期限監視）　★実施済み（2026-07-03）
- 参照: 04 §1・§4〜§6
- 作業: CHANNEL_REGISTRY 登録・投函後の状態遷移・追跡番号運用・統合テスト
- 完了条件:
  - [x] 起票→承認→発送済→返送待ち→（期限超過警報）の統合テスト PASS
  - [x] 冪等（二重 Webhook）テスト PASS・既存テスト全 PASS（337 passed / 2 skipped）
- 実装ノート（2026-07-03）:
  - channels/__init__.py で ShokumuSeikyuAdapter を register。**App 30 の
    「チャネル=職務上請求」の起票がこのマージから実際に動く**（M4 送付案内に続き2本目）
  - hub/dispatch に **発送済ハンドラ（_handle_shipped）を新設**。物理郵送チャネルで人が
    発送済に変更→Webhook 再発火→返送要否を判定して自動遷移:
    返送あり→返送待ち＋返送期限（発送日＋UNIT_CONFIG.return_deadline_days・既定21日）/
    返送なし→完了（SERVER_TRANSITIONS の設計どおり）
  - 返送要否の判定は「アダプタ属性 needs_return（M1=True）→ チャネル固有データの
    needs_return フラグ（M4 がレコード単位で記録）」の順。**T2-2 実装ノートで予告した
    「M4 の返送待ち自動遷移も T3-3 でまとめて扱う」を実施**（M4 も自動化された）
  - 承認済→発送処理中→（印刷投函 LINE）は M4 と同型の既存経路をそのまま使用（変更なし）
  - 返送期限監視（T1-4）は 返送待ち をチャネル横断で検索するため職務上請求も自動対象。
    検知テストを test_shokumu_wiring.py に追加
  - M5 受領（将来の T4系）との接続点は _handle_shipped と ShokumuSeikyuAdapter.dispatch の
    docstring に明記（返送待ち→完了 の消込は M5・突合不能は 要確認→reprocess）
  - **既存テストへの影響（設計予告済みの仕様変更に伴う2箇所・アサーション趣旨の変更は結線の
    事実を反映したもののみ）**: ①test_hub_dispatch の「処理対象外ステータス一覧」から
    発送済 を除外（発送済が処理対象になったため）②test_shokumu_seikyu の
    「未登録」検証を「登録済み」検証に反転（T3-1 時点の暫定検証の期限到来）
  - テストは test_shokumu_wiring.py（10件・新規: 結線・一巡・二重 Webhook・
    発送済分岐3種・期限監視検知）
