...
- 照合源は**実在するもののみ**: `external_event_id`（LINE webhookEventId）・既存の構造化ログ・
  LINE 側の配信記録等。
- **証明不能な行の扱い（fix3 M02・§12.1 と逐語統一）**: **照合源の探索・照合を実施しても
  正常処理済みを証明できない行は、§12.1 の分岐 4（別管理付き残置＝一覧を work-log に
  固定し、照合源が得られ次第 再判定）のみ**が許される扱いである
  （根拠なき done 更新＝未処理の処理済み偽装は禁止。「残置」は照合の実施を前提とした
  帰結であって、照合の省略を許す規定ではない）。
