...
def run_check(snapshot_dir: Path, repo_root: Path, show_content: bool = False,
    ...
    if not (repo_root / "gas").is_dir():
        print(f"[error] repo-root 不正: {repo_root} に gas/ がありません"
              "（--repo-root を確認してください）", file=out)
        return 2

    try:
        snaps = sorted(p for p in Path(snapshot_dir).glob("*.js") if p.is_file())
    except OSError:
        print(f"[error] snapshot ディレクトリを読み取れません: {snapshot_dir}", file=out)
        return 2
    if not snaps:
    ...
    if _REQUIRED_SNAPSHOT not in {p.name for p in snaps}:
        print(f"[error] 必須ファイル {_REQUIRED_SNAPSHOT} が snapshot にありません"
              "（SIGNED_LANES 比較不能のため検査を中断します）", file=out)
        return 2
    ...
    for snap in snaps:
        ...
        if name_secret:
            print("[warn] secret 様の文字列を含むファイル名の snapshot を検出"
                  "（名前は表示しません）。改名のうえ再実行を推奨します。", file=out)

        # M01: 読取不能・decode 失敗は固定文言＋exit 2（traceback にしない）
        try:
            snap_text = snap.read_text(encoding="utf-8")
        except UnicodeDecodeError:
            print(f"[error] {disp}: UTF-8 として読めません（貼付内容を確認してください）",
                  file=out)
            return 2
        except OSError:
            print(f"[error] {disp}: 読み取りに失敗しました", file=out)
            return 2

        # secret 様パターンの防御（該当行の内容は一切出さない）
        secret_lines = find_secret_lines(snap_text)
        if secret_lines:
            print(f"[warn] {disp}: secret 様パターンを検出（行 {secret_lines}）。"
                  "該当行の内容は表示しません。誤貼付でないか確認し、"
                  "secret は snapshot に含めないでください。", file=out)

        repo_file = _find_repo_file(repo_root, snap.name)
        if repo_file is None:
            print(f"[drift] {disp}: repo 側（gas/・legacy/gas/）に対応ファイルなし"
                  "（manifest 余分）", file=out)
            drift = True
            continue
        ...
        try:
            ...
            print(f"[error] repo 側 {repo_disp} を読み取れません", file=out)
            return 2

        mismatched = _diff_line_numbers(repo_text, snap_text)
        if mismatched:
            drift = True
            print(f"[drift] {disp}: 不一致 {len(mismatched)} 行 "
                  f"(repo={repo_disp}) 行番号: {mismatched[:50]}", file=out)
            if show_content:
            ...
            print(f"[ok]    {disp}: repo と一致 (repo={repo_disp})", file=out)
    ...
    for repo_js in sorted((repo_root / "gas").glob("*.js")):
        if repo_js.name not in seen_names:
            print(f"[drift] gas/{repo_js.name} が snapshot にありません"
                  "（manifest 不足・live との照合が未完）", file=out)
            drift = True
