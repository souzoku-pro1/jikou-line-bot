...
### 発送/受領ハブ（M4 送付案内）の実地一巡手順（T2-2・2026-07-03 追加）

kintone 画面だけで「起票→承認→印刷指示」の一巡を確認する手順。

**前提（済んでいなければ先に）**:
1. `HUB_WEBHOOK_TOKEN` の Railway 登録と App 30 の Webhook 設定（App 30 作成手順書 §4）
2. 事務所情報の Railway 登録: `OFFICE_NAME` / `OFFICE_ZIP` / `OFFICE_ADDRESS` / `OFFICE_TEL`
   （未設定だと prepare が「事務所情報が未設定」エラーになる＝仕様）
3. 同封物マスタの初期登録（**この順序で**・docs/architecture/02 §4.2）:
   ① App 30 の「同封物選択」フィールドに選択肢を追加（例: `委任契約書2通`・`返信用封筒`）
   ② App 32 に同じ `ブロックキー` でレコードを追加（表示名・案内文・対象ユニット=時効援用・
      返送要否・表示順・有効=yes）→ どちらも「アプリを更新」

**一巡テスト**:
1. App 30 で新規レコード作成: ユニット種別=時効援用 / チャネル=送付案内 / 方向=発送 /
   件名=テスト送付 / 宛先名・宛先郵便番号・宛先住所=**事務所自身の宛先**（誤送防止）/
   同封物選択にチェック → 保存（発送ステータスは「下書き」のまま）
2. 数秒後にレコードを開き直す → 発送ステータスが**「承認待ち」**になり、
   成果物に **送付案内.docx と 宛名ラベル.pdf** が添付され、
   本文_特記事項に AI の一文が入っていることを確認（弁護士 LINE に【承認依頼】発送 が届く）
3. docx を開いて内容確認（修正したい場合: 本文_特記事項を直して発送ステータスを
   「下書き」に戻すと再生成される）
4. 発送ステータスを**「承認済」**に変更して保存
5. 数秒後: 発送ステータスが**「発送処理中」**・実行済み=yes になり、
   LINE に【印刷・投函をお願いします】が届くことを確認
6. （実運用では印刷・封入・投函 →）発送ステータスを「発送済」→「完了」に手動変更して終了
   ※ テストレコードは「完了」にして残す（削除しない・監査痕跡）

**うまくいかないとき**: 発送ステータスが「エラー」になった場合は エラー詳細 欄と
LINE 警報の文言を確認（未定義ブロックキー・事務所情報未設定・テンプレート不備が典型）。
修正後に発送ステータスを「下書き」へ戻せば自動で再処理される。
