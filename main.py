...
# ── P4-001/P4-002/P4-004: PWA（仮名称・認証+shell+案件画面+承認キュー参照）。
#    末尾追記は sink allowlist の行番号 pin を壊さないための規約（health_deps と
#    同型・pin 最大行より後）。P4-002/P4-004 の router は webapp_auth の catch-all
#    （/app/{_rest:path}）より**先に**結線する必要がある（include 順=マッチ順）。
#    認証はいずれも _gate（P4-001）が担う。
