...
# ── 費用の定型案内（固定文） ────────────────────────────────────────────────────
# TODO(LP確認): 税込/税抜の表記は現行LPの料金表と一致させること。
#   2026-07-03 時点でLPの所在が特定できず未確認。弁護士の確認を得てから表記を確定する。
FEE_GUIDE_TEXT = (
    "【費用のご案内】\n"
    "・費用: 1社あたり44,000円（複数社の場合は 44,000円 × 社数）\n"
...
FEE_REQUIRED_PHRASES = ["44,000円", "前払い", "分割払い", "費用は発生"]
...
# 「必ず消滅」等は、直後に否定（〜とは限らない/保証できない/言い切れない 等）が
# 続く場合を除外する。法律知識ブロックの必須文言「必ず消滅するとは保証できない」が
# 誤検出されるため（2026-07-03 実測で確認・弁護士承認済みの緩和）。
_FORBIDDEN_PATTERNS: list[tuple[str, re.Pattern]] = [
    (
        "断定語",
        re.compile(
            r"確実に|絶対に|間違いなく"
            r"|必ず(?:消滅|成立|時効)(?![^。]{0,6}とは(?:限|保証|言|い|断|申))"
            r"|100[%％]|１００[%％]"
        ),
    ),
...
- 記録にない進捗・日付・金額の創作は禁止
- 断定語（確実に/絶対に/間違いなく/必ず 等）・行動指示語（払わないで/無視して/連絡しないで/放置して 等）は使用禁止。唯一の例外は【FAQ】記載の受任後顧客への電話対応の定型指示
...
- 「絶対に大丈夫か」「もう払わなくていいか」など断定や行動指示を求められた場合 → カテゴリ「法的判断・見通し」
...
- 業者からの電話:
  ...
  * ※ 電話ではなく、業者からの通知・請求・書面について「無視していいか」「放置していいか」「払わなくていいか」等の対応可否を問われた場合は、フェーズを問わず承認制（カテゴリ「法的判断・見通し」、auto_send=false）。通知が届いた事実は弁護士が確認する必要がある
...
- 裁判所書類・差押えなど緊急連絡の場合（カテゴリ「緊急対応」。第一報なら immediate_notice="court_doc_request"）
- 業者からの通知・請求・書面への対応可否（無視していいか・放置していいか・払わなくていいか等）を問われた場合（カテゴリ「法的判断・見通し」）
